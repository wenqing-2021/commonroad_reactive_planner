...
class CostFunction(ABC):
    ...
    @abstractmethod
    def evaluate(
        self, trajectory: commonroad_rp.trajectories.TrajectorySample
    ) -> float:
        """
...
class DefaultCostFunction(CostFunction):
    ...
    def evaluate(self, trajectory: commonroad_rp.trajectories.TrajectorySample):
        ...
        costs += (
            ...
            + (200 * (self.desired_d - trajectory.curvilinear.d[-1])) ** 2
        )
        # orientation costs
        costs += (
...
class DefaultCostFunctionFailSafe(CostFunction):
    ...
    def evaluate(self, trajectory: commonroad_rp.trajectories.TrajectorySample):
        ...
        # distance costs
        costs += (
            np.sum((0.25 * trajectory.curvilinear.d) ** 2)
            + (20 * trajectory.curvilinear.d[-1]) ** 2
        )
        # orientation costs
        costs += (
            np.sum((0.25 * np.abs(trajectory.curvilinear.theta)) ** 2)
